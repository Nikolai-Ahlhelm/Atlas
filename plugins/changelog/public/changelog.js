(() => {
  ...
  function fillSettingsForm() {
    ...
    state.tagSuggestionsDraft = Array.isArray(state.detail.tagSuggestions) ? [...state.detail.tagSuggestions] : [];
    ...
    form.elements.tag_suggestions.value = state.tagSuggestionsDraft.join(', ');
  }
  ...
  function renderEntryTable() {
        ...
        <table class="changelog-table spreadsheet-table">
          ...
          <tbody>
            ${items.length ? items.map((item, index) => `
              <tr class="${state.editor.entryId === item.id ? 'is-active' : ''}">
                <td class="row-index-cell">${(state.page - 1) * state.pageSize + index + 1}</td>
                ${visibleColumns.map((column) => `<td>${renderCellValue(column, getEntryCellValue(item, column))}</td>`).join('')}
                ${state.detail.can?.edit ? `<td class="row-actions">${item.canEdit ? `<button class="button small ${state.editor.entryId === item.id ? 'primary' : 'ghost'}" type="button" data-edit-entry="${item.id}">${esc(msg('edit', 'Edit'))}</button>` : ''}</td>` : ''}
              </tr>
            `).join('') : `
    ...
    target.querySelectorAll('[data-edit-entry]').forEach((button) => button.addEventListener('click', () => {
  ...
  function handleGlobalClick(event) {
    ...
    if (rerenderEditor) renderEntryEditor();
  }

  function renderCellValue(column, value) {
    if (value === null || value === undefined || value === '') return '<span class="hint">-</span>';
    if (Array.isArray(value)) return `<div class="chip-list">${value.map((item) => `<span class="pill">${esc(item)}</span>`).join('')}</div>`;
  ...
  function renderEntryEditor() {
    ...
    if (!target || !state.detail || !state.detail.can?.edit) return;
    ...
    target.hidden = false;
    target.innerHTML = `
      <form id="changelogEditorForm" class="panel changelog-editor-panel">
        <div class="panel-head compact">
          <div>
            <h2>${esc(state.editor.mode === 'edit' ? msg('editEntry', 'Edit entry') : msg('newEntry', 'New entry'))}</h2>
            <p class="hint">${esc(state.editor.mode === 'edit' ? msg('editEntryHint', 'Adjust the row and save directly below the table.') : msg('newEntryHint', 'Add a new row without leaving the list view.'))}</p>
          </div>
          <div class="panel-head-actions">
            <button class="button ghost" type="button" id="cancelEntryEditor">${msg('cancel', 'Cancel')}</button>
            <button class="button primary" type="submit">${msg('save', 'Save')}</button>
          </div>
        </div>
        <div class="changelog-editor-grid">
          ${columns.map((column) => renderEntryField(column, state.editor.values[column.key])).join('')}
        </div>
        <div class="editor-meta-row">
          ${state.editor.mode === 'edit' ? `<span class="hint">${esc(msg('editingExistingEntry', 'Editing existing entry'))}</span>` : `<span class="hint">${esc(msg('creatingNewEntry', 'Creating new entry'))}</span>`}
        </div>
      </form>
    `;
    document.querySelector('#cancelEntryEditor')?.addEventListener('click', closeEntryEditor);
    document.querySelector('#changelogEditorForm')?.addEventListener('submit', saveEntryEditor);
    bindEntryEditorEvents();
  }
  ...
  function openEntryEditor(entry = null) {
    ...
    (state.detail?.columns || []).forEach((column) => {
      ...
      state.editor.values[column.key] = value ?? fallback;
    });
    renderEntryEditor();
    document.querySelector('#changelogEntryEditor')?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
  }
      ...
      return `
        <div class="entry-picker-field">
          ...
          <button class="entry-picker-trigger" type="button" data-entry-picker="${esc(column.key)}">
            <span>${esc(selected.length ? `${selected.length} ${msg('selected', 'ausgewaehlt')}` : msg('selectTags', 'Auswaehlen'))}</span>
            <span class="sheet-filter-glyph">▾</span>
          </button>
          ${isOpen ? `
            <div class="entry-picker-menu sheet-column-menu" data-entry-picker-panel="${esc(column.key)}" style="${inlineStyle(state.activeEntryPickerStyle)}">
              <fieldset class="sheet-menu-checklist">
                <legend>${esc(column.label)}</legend>
                <div class="sheet-menu-checklist-body">
                  ${options.length ? options.map((option) => `
                    <label class="check sheet-menu-check-option">
                      <input data-picker-key="${esc(column.key)}" name="${esc(fieldName)}" type="checkbox" value="${esc(option)}" ${selected.includes(option) ? 'checked' : ''}>
                      <span>${esc(option)}</span>
                    </label>
  ...
  async function saveEntryEditor(event) {
    ...
    const payload = {
      ...
      values: readEntryValues(form, state.detail.columns || [])
    };
    try {
  ...
  function readEntryValues(form, columns) {
    const values = {};
    columns.forEach((column) => {
      ...
      if (!element) return;
      if (element.type === 'checkbox') values[column.key] = element.checked;
      else values[column.key] = element.value;
    });
    return values;
  }

  function bindEntryEditorEvents() {
    const form = document.querySelector('#changelogEditorForm');
    if (!form || !state.detail) return;
    (state.detail.columns || []).forEach((column) => {
      const fieldName = `field_${column.key}`;
      ...
      elements.forEach((element) => {
        ...
        element.addEventListener(eventName, () => syncEditorFieldValue(form, column));
      });
    });
    form.querySelectorAll('[data-entry-picker]').forEach((button) => button.addEventListener('click', (event) => {
      event.stopPropagation();
      const key = button.dataset.entryPicker;
      if (state.activeEntryPicker === key) {
        state.activeEntryPicker = '';
        state.activeEntryPickerStyle = null;
      } else {
        state.activeEntryPicker = key;
        state.activeEntryPickerStyle = buildFloatingMenuStyle(button.getBoundingClientRect(), { preferredWidth: 320, preferredHeight: 360 });
      }
      renderEntryEditor();
    }));
    form.querySelectorAll('[data-picker-key]').forEach((input) => input.addEventListener('change', () => {
      const key = input.dataset.pickerKey;
      state.editor.values[key] = Array.from(form.querySelectorAll(`input[data-picker-key="${cssEscape(key)}"]:checked`)).map((item) => item.value).filter(Boolean);
      renderEntryEditor();
    }));
    form.querySelectorAll('[data-close-entry-picker]').forEach((button) => button.addEventListener('click', () => {
      state.activeEntryPicker = '';
      state.activeEntryPickerStyle = null;
      renderEntryEditor();
    }));
  ...
  function syncEditorFieldValue(form, column) {
    const fieldName = `field_${column.key}`;
    const element = form.elements[fieldName];
    if (!element) return;
  ...
  function renderTagSuggestionManager() {
        ...
        <div class="changelog-tag-manager-section">
          ...
          <div class="chip-list">
            ${activeTags.length ? activeTags.map((tag) => `
              <button class="pill removable" type="button" data-remove-tag-suggestion="${esc(tag)}">
                <span>${esc(tag)}</span>
        ...
        <div class="changelog-tag-manager-section">
          ...
          <div class="chip-list">
            ${knownTags.length ? knownTags.map((tag) => `
              <button class="pill ${activeTags.includes(tag) ? 'is-active' : ''}" type="button" data-import-tag-suggestion="${esc(tag)}" ${activeTags.includes(tag) ? 'disabled' : ''}>
                ${esc(tag)}
              </button>
    ...
    `;
    form.elements.tag_suggestions.value = activeTags.join(', ');
  ...
  function addTagSuggestion(value) {
  ...
  function renderAnalytics() {
    ...
    const html = state.analytics ? `
      <div class="changelog-metrics">
        <article class="metric-card"><span>${esc(msg('totalEntries', 'Total entries'))}</span><strong>${state.analytics.totalEntries}</strong></article>
        <article class="metric-card"><span>${esc(msg('recentChanges', 'Recent changes (14d)'))}</span><strong>${state.analytics.recentChanges}</strong></article>
        <article class="metric-card"><span>${esc(msg('lastUpdate', 'Last update'))}</span><strong>${esc(formatDateTime(state.analytics.latestUpdatedAt))}</strong></article>
      </div>
  ...
  function renderAnalyticsList(title, items) {
    return `
      <section class="panel analytics-panel">
        <div class="panel-head compact"><h2>${esc(title)}</h2></div>
        ...
        ` : `<p class="hint">${esc(msg('noData', 'No data yet.'))}</p>`}
      </section>
    `;
  }
    ...
    try {
      const result = await fetchJson('/api/admin/changelogs/list', {
        ...
        body: JSON.stringify({
          ...
          tag_suggestions: Array.isArray(state.tagSuggestionsDraft) ? state.tagSuggestionsDraft : parseCsv(form.elements.tag_suggestions.value)
        })
      });
      await refreshAdmin();
