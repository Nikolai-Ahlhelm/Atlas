(() => {
  ...
  function renderField(field) {
    ...
    if (field.type === 'divider') {
      return `
        <section class="form-section-divider"${visibility}>
          <hr>
          <span>${esc(field.label || msg('section', 'Section'))}</span>
        </section>
      `;
    }
    ...
    if (field.type === 'textarea') {
      return `<label data-form-field="${esc(field.key)}"${visibility}>${esc(field.label)}${hint}<textarea name="${esc(field.key)}" placeholder="${esc(field.placeholder || '')}" ${required}></textarea></label>`;
    }
    if (field.type === 'select') {
      return `
        <label data-form-field="${esc(field.key)}"${visibility}>${esc(field.label)}${hint}
          <select name="${esc(field.key)}" ${required}>
            <option value="">${esc(msg('pleaseChoose', 'Please choose'))}</option>
    ...
    if (field.type === 'checkbox') {
      return `
        <label class="check" data-form-field="${esc(field.key)}"${visibility}>
          <input name="${esc(field.key)}" type="checkbox">
    ...
    if (!currentSubmissions.length) {
      target.innerHTML = `
        <section class="forms-submit-card">
          ...
          <p class="hint">${msg('noSubmissions', 'No submissions yet.')}</p>
        </section>
      `;
      return;
    }
              ...
              <dl class="forms-submission-values">
                ${submission.values.map((item) => `
                  <div>
                    ...
                    <dd>${esc(formatValue(item.value))}</dd>
                  </div>
                `).join('')}
              </dl>
  ...
  function renderReviewForm(submission) {
    return `
      <form class="forms-review-form" data-review-submission="${submission.id}">
        <input type="hidden" name="submission_id" value="${submission.id}">
        <label>${msg('status', 'Status')}
          <select name="status">
            ${['submitted', 'in_review', 'approved', 'rejected'].map((status) => `<option value="${status}" ${submission.status === status ? 'selected' : ''}>${esc(status)}</option>`).join('')}
          </select>
        </label>
        <label>${msg('notes', 'Notes')}<textarea name="notes">${esc(submission.notes || '')}</textarea></label>
        <div class="modal-actions">
          <button class="button primary" type="submit">${msg('saveReview', 'Save review')}</button>
        </div>
      </form>
    `;
  }
  ...
  async function submitReview(event) {
    ...
    try {
      await fetchJson('/api/forms/submissions/review', {
        ...
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          submissionId: Number(form.get('submission_id')),
